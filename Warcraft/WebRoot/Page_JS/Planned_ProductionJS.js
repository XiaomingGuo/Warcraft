...
function changePOName()
{
    ...
    $.post("Ajax/Query_Planning_PO_Item_Ajax.jsp", {"po_name":po_name}, function(data, textStatus)
    {
            ...
            if (iColCount > 0&&iRowCount > 0)
            {
                ...
                for(var iRow = 1; iRow <= iRowCount; iRow++)
                {
                    ...
                    for (var iCol = 1; iCol <= iColCount; iCol++)
                    {
                        ...
                        if (0 == iColCount - iCol)
                        {
                            td.append("<input type='button' value='投产' name='" + data_list[iRow*iColCount + 7] + "$" + data_list[iRow*iColCount + 8] + "$" + data_list[iRow*iColCount + 3] + "' id='" +data_list[iRow*iColCount + 3] + "_Button' onclick=DoTransfer(this)>");
                        }
...
function DoTransfer(obj)
{
    ...
    $.post("Ajax/Transfer_To_SemiProduct_Ajax.jsp", {"Bar_Code":tempList[0], "PO_Name":tempList[1], "TransQty":storeQTY}, function(data, textStatus)
    {
        ...
        location.href ="Planned_Production.jsp?POName="+data.split('$')[1];
    });
}
