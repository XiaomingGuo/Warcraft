...
import com.DB.support.ManuStorageRecord;
...
public class Manu_Storage_Record extends DBTableParent implements ITableInterface, IStorageTableInterface
{
	...
	public List<String> getDBRecordList(String keyWord)
	{
		...
		Iterator<ManuStorageRecord> it = resultList.iterator();
		while(it.hasNext())
		{
			ManuStorageRecord tempRecord = (ManuStorageRecord)it.next();
			switch (keyWord)
			{
			case "id":
	...
	{
		String hql = String.format("from ManuStorageRecord cpr where cpr.%s='%s' order by cpr.%s asc", GetDatabaseKeyWord(keyWord), value, GetDatabaseKeyWord(orderKey));
	...
	{
		String hql = String.format("from ManuStorageRecord cpr where cpr.%s='%s' order by cpr.%s desc", keyWord, value, orderKey);
		getEQMHandle().EQQuery(hql);
	}
	*/
	...
	public void DeleteRecordByKeyWord(String keyWord, List<String> delList)
	{
		...
		{
			String hql = String.format("delete ManuStorageRecord por where por.%s='%s'", GetDatabaseKeyWord(keyWord), item);
	...
	{
		aWriteRecord = new ManuStorageRecord();
	...
	{
		String hql = "from ManuStorageRecord ms where ";
