...
public class SubmitDiscardMaterial extends PageParentClass
{
	...
	{
		DBTableParent hHandle = GenStorageHandle(appBarcode);
	...
	public boolean ExcuteDiscardMaterial(String appBarcode, String appPOName, String appOperator, String appreason, int used_count)
	{
		...
		DBTableParent hHandle = GenStorageHandle(appBarcode);
		int repertory_count = hHandle.GetRepertoryByKeyList(Arrays.asList("Bar_Code", "po_name"), Arrays.asList(appBarcode, appPOName));
		if (repertory_count >= used_count)
		{
			...
			for (int iCol = 0; iCol < material_info_List.get(0).size(); iCol++)
			{
				...
				{
					((Discard_Material_Record)hDMRHandle.getTableInstance()).AddARecord(appPOName, strWriteBarCode, batchLot, appOperator, Integer.toString(used_count), appreason);
				...
				{
					((Discard_Material_Record)hDMRHandle.getTableInstance()).AddARecord(appPOName, strWriteBarCode, batchLot, appOperator, Integer.toString(recordCount), appreason);
					if(sql_out_count == 0)
