...
<%
	...
	if (appPOName != null && appDelivDate.length() == 8)
	{
		...
		List<List<String>> recordList = hPageHandle.GetCustomerPoRecordList(appPOName, appVendor);
		if (recordList.size() > 0)
		{
			...
			for (int iRow = 0; iRow < recordList.get(0).size(); iRow++)
			{
				...
				{
					hMMPHandle.QueryRecordByFilterKeyList(Arrays.asList("Bar_Code", "vendor", "po_name"), Arrays.asList(strBarcode, strVendor, appPOName));
					if (hMMPHandle.RecordDBCount() <= 0)
					{
						if (appDelivDate != null&&!appDelivDate.isEmpty()&&appDelivDate.length() == 8)
						{
							hMMPHandle.AddARecord(strBarcode, strVendor, appPOName, manufacture_QTY-iRepertory, appDelivDate);
						}
						else
						{
