			...
			if (hDBHandle.QueryDataBase(sql))
			{
				...
				for (int iCol = 0; iCol < material_info_List.get(0).size(); iCol++)
				{
					...
					if (recordCount >= used_count)
					{
						...
						sql = "INSERT INTO shipping_record (customer_po, Bar_Code, Batch_Lot, Order_Name, ship_QTY) VALUES ('" + appPONum + "','" + appBarcode + "','" + batchLot + "','" + ordername +"','" + used_count +"')";
						hDBHandle.execUpate(sql);
						break;
					}
					else
					{
						...
						hDBHandle.execUpate(sql);
						hDBHandle.MoveToExhaustedTable(appBarcode, batchLot, "product_storage", "exhausted_product");
						sql = "INSERT INTO shipping_record (customer_po, Bar_Code, Batch_Lot, Order_Name, ship_QTY) VALUES ('" + appPONum + "','" + appBarcode + "','" + batchLot + "','" + ordername +"','" + Integer.toString(recordCount) +"')";
						hDBHandle.execUpate(sql);
				...
				sql= "UPDATE customer_po_record SET OUT_QTY='" + writeQTY + "'WHERE Bar_Code='" + appBarcode +"' and po_name='" + appPONum +"'";
				hDBHandle.execUpate(sql);
